...
09_MULTI_MUNICIPALITY.md       ← the second municipality (Ramtha), part by part, 0111–0133
...
| `v_rmth_indicator_unique` (0132) | a view, a passing probe through the save RPC, and a `counted_under_id` the RPC derives correctly the day a record is saved | agreement with the rows after any later edit. The view counted enrolments whose derived field was null; withdraw the earlier completion and the later one still pointed at it. One person on three cycles read as three unique people the first time the probe skipped the RPC. `0133` counts people from the spine, as the other person-level views already did |
| `handle_new_user` reading app metadata (0117) | a trigger that shapes the account from `raw_app_meta_data`, and a probe in `0118` that created two accounts that way and passed every isolation test | GoTrue. The admin API inserts the auth row **first** and applies `app_metadata` **afterwards**, so the AFTER INSERT trigger only ever sees `{provider, providers}`. The probe inserted the row the way I imagined GoTrue does, not the way it does. The first real `manage-account` call made a participant with no municipality; the function's read-back, not the test, caught it. **A fixture that reproduces your model of an external system tests your model.** Drive the real API once |
| The E0.3 cycle picker | a picker over incubator-design cycles, correctly filtered, on the one form that needs it, with the four cycle columns beside it | anything to pick. No form created an incubator-design cycle: C1.1 makes employability cycles, F0.2's log makes deliveries. The form could never be saved. Found by trying to save it |
...
| The required-field guard in `RmthFormScreen` | `if (invalid) return` at the top of `submit()`, after `setTouched(true)`, with every rule written and every message translated | a value computed in THIS click. The errors were computed during render only once `touched` was true, and `submit()` read `invalid` from the render in which it was still false — so the **first** click on any incomplete Ramtha form went to the database with the blanks in it, the not-null refusal came back as raw SQL, and "this field is required" appeared a render later. The second click stopped correctly, which is why it looked like it worked. Found by saving a C1.1 cycle with only a title |
| `confirm` in the `evidence` function | a HEAD of the object, a comparison of `Content-Length` with the declared size, and a delete of the object on a mismatch — the shape of a careful check | the header. Deno's fetch handed the HEAD response back without `content-length`; `Number(null)` is 0; a 186-byte object the store had answered 200 and a content ETag for was declared a mismatch and **deleted**. The check compared a number that was not the object's, and its failure mode destroyed the thing it was verifying. It asks the store with a one-byte ranged GET now. **A check whose false negative is destructive must be sure of its input** — and this one was first run against the real store on the day it was needed, not the day it was written |
| `presign_upload` in the `evidence` function | the record read as the user before any URL was signed, so an invisible record — not theirs, not there — got `not_found` and no URL; RLS as the permission check, exactly as the header promised | `deleted_at`. A soft-deleted record is VISIBLE to its municipality (its screen shows it, with Restore), so RLS let the read through and the function signed an upload for a record the plan does not count; the panel offered "Add a file" under a banner saying the record does not count. Found by asking for one. `record_deleted` now, and no Add on a deleted record. **Visibility is not the same question as eligibility**, and a function that delegates the first to RLS still owes the second. Neither this nor the row above could have appeared against a mocked store or a fixture: both were found by driving the real one |
| `types/database.ts` "still stale" and TS2589 "probably the embed hints" | a diagnosis handed in with the task, a file that had been stale twice before, and an error whose message says "possibly infinite" | either half. A fresh generation at head 0135, stripped, was byte-identical to the committed file; and the error was not recursion in the hints or in anything of ours but **depth** in a dependency -- `DeduplicateRelationships` in postgrest-js walks a table's `Relationships` tuple one frame per entry, TypeScript stops a conditional type at 100 frames, and the generator's view entries put 42-78 on the scoped tables. 1.24 M instantiations raw against 0.96 M stripped, both far under the 5 M cap. Unchanged in the latest release. **A diagnosis that arrives with the task is a claim to check, not a premise**, and a dependency's type-level limit is a real constraint: "regenerate it" is not always the answer, and "it is recursing" is a description of the message, not of the cause |
...
- **Never write a comment claiming a behaviour is implemented somewhere else.** Comments explain reasoning. Behaviour is proven by a test or by running it.
  ...
  This has now happened three times. `format.ts` carried a constant that read as authoritative about Western digits while two of three code paths ignored it. `glyphs.ts` said the back arrow was "mirrored under RTL by the `scale-x-[-1]` on the span that renders it" — and not one of the four call sites did that. Both comments were written in good faith, described a real intention, and were false. And `0113`'s header said "0116 adds `indicator.full_code`" — 0116 added no such column, three more migrations went by, and it was found when `0131` needed the column and the catalogue did not have it. A comment that promises a *future* migration is the same comment with a longer fuse.
