 ...
 *  today, and every one of those values comes from a SEEDED demo row. No form
 *  in this platform collects refugee status or disability — the fields were
 ...
 * ─────────────────────────────────────────────────────────────────────────────
 */
export function DisaggregationPanel({ periodCode }: { periodCode: string | undefined }) {
  const { t } = useTranslation(['indicators', 'common'])
  const q = useDisaggregation(periodCode)
  const [code, setCode] = useState<string>(DISAGGREGABLE_CODES[0])
  const rows = q.data ?? []
  ...
  return (
    <section className="mt-8">
      ...
      {/* ── OQ-12, stated before any figure is shown. ── */}
      <div
        role="note"
        className="mt-3 border-s-[3px] border-amber bg-attention-bg p-4"
      >
      ...
      <p className="mt-3 max-w-[74ch] text-[14px] leading-[1.5] text-muted">
        {t('indicators:disagg.intro')}
      </p>

      {/* Which of the four. Only these have a person behind every unit. */}
      <div className="mt-4 flex flex-wrap gap-2">
        {DISAGGREGABLE_CODES.map((c) => (
          <button
            key={c}
            type="button"
            onClick={() => setCode(c)}
      ...
      {q.isLoading ? (
        ...
        <p className="mt-4 border-[1.5px] border-dashed border-border-muted bg-sunken p-4 text-[14px] leading-[1.5] text-muted">
          {t('indicators:disagg.noneInPeriod', { code, period: periodCode ?? '' })}
        </p>
      ) : (
        <>
          <p className="mt-4 font-narrow text-[12px] font-bold uppercase tracking-[0.1em] text-muted">
            {t('indicators:disagg.totalLine', { code, count: total })}
          </p>
          <div className="mt-2 grid grid-cols-1 gap-x-10 gap-y-6 md:grid-cols-2">
            {DIMENSIONS.map((dim) => (
              <DimensionBlock key={dim} dim={dim} rows={tally(rows, code, dim)} total={total} />
            ))}
          </div>
        </>
      )}
...
function DimensionBlock({
  ...
  const uncollected = UNCOLLECTED_DIMENSIONS.includes(dim)
